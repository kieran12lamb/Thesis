...
def table_chunk(i):
    align_chunk=align[i:i+args.chunk]
    nucleotides = ["A","C","G","T","-"]
    chunk_dict = {}
    masks = np.array([np.full_like(np.array(reference.seq, dtype="string_"),nucleotide, dtype="string_") for nucleotide in nucleotides ], dtype="string_")
    for seq in align_chunk:
        lineage = seq.id.split("|")[4]
        full_lineage = seq.id.split("|")[5]
        sequence_matrix = np.array(([np.array(seq.seq, dtype="string_")] * len(nucleotides)), dtype="string_")
        bool_matrix = (sequence_matrix == masks).astype(int)
        data = pd.DataFrame(bool_matrix,columns=[np.arange(len(reference))],index=nucleotides)
        data.index = nucleotides
        data.index.name =full_lineage
#         print(data)
        if lineage in chunk_dict:
            chunk_dict[lineage] = chunk_dict[lineage].add(data)
        else:
            chunk_dict[lineage] = data
    return chunk_dict
...
def reference_chunk(i):
    ...
    for lineage in ref_chunk:
        ...
        if "/" in lineages_dictionary[lineage].index.name:
            ...
            continue
        pseudo_reference = ""
        for col in list(lineages_dictionary[lineage].columns):
                ...
                if "." not in lineage:
                    ...
                    for j in range(len(lin)-1,0,-1):
                        ...
                        if parent_lineage in reference_dict:
                            wt = reference_dict[parent_lineage].seq[int(col)]
                            print(wt)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-a',"--alignment", metavar='str', type=str,help='Alignment File Path')
    ...
    multiAlignmentPath = args.alignment
    reference = SeqIO.read(args.reference, "genbank")
    align = AlignIO.read(args.alignment, "fasta")
    seq_count = int(len(align))
    j = 0
    time = datetime.now()
    lineages_dictionary = {}
    total_blocks = math.ceil(seq_count/args.block)
    
    while(j<seq_count):
        print(f'Block {j/args.block}/{total_blocks}, Time Taken: {datetime.now()-time}')
        if seq_count > j+args.block:
            align = AlignIO.read(multiAlignmentPath, "fasta")[j:j+args.block]
        else:
            align = AlignIO.read(multiAlignmentPath, "fasta")[j:seq_count]
        print(f'Block loaded :{datetime.now()-time}')
        i=0
        total_chunks = math.ceil(len(align)/(args.chunk*args.threads))
        while(i<len(align)):
            chunk_time = datetime.now()
            pool = multiprocessing.Pool(args.threads)
            if len(align)> i+(args.chunk*args.threads):
                results = pool.map(table_chunk,np.arange(i,i+(args.chunk*args.threads),args.chunk))
            else:
                length = len(align)
                results = pool.map(table_chunk,np.arange(i,length,args.chunk))
            pool.close()
            pool.join()
            for dictionary in results:
                dictionary_keys =list(dictionary.keys()) 
                for lineage in dictionary_keys:
                    if lineage in lineages_dictionary:
                        lineages_dictionary[lineage] = lineages_dictionary[lineage].add(dictionary[lineage])
                    else:
                        lineages_dictionary[lineage] = dictionary[lineage]
            i=i+(args.chunk*args.threads)
            print(f'Chunk {i/(args.chunk*args.threads)}/{total_chunks}, Time Taken: {datetime.now()-chunk_time}')
        j+=args.block
        
    
    all_lineages = list(lineages_dictionary.keys())
    all_lineages = sorted(all_lineages)
    
    print(all_lineages)
    
    reference_dict = {}
    print("And now the slow bit")
    total_chunks = math.ceil(len(all_lineages)/args.chunk)
    ...
    for lineage in all_lineages:
        ...
        lineage_counts = lineages_dictionary[lineage].copy(deep=True)
        lineage_counts["lineage"] = lineage
        lineage_counts.index.name = None
        counts_table.append(lineage_counts.reset_index().rename(columns={'index': 'nucleotide'}))
    counts_table = pd.concat(counts_table,axis=0)
    counts_table.to_csv(f'{args.output.split(".")[0]}_counts.csv',index=False)
    
    counts_table =pd.read_csv(f'{args.output.split(".")[0]}_counts.csv',index_col=False)
    print(counts_table)
    all_lineages = counts_table.lineage.unique()
    ...
    for lineage in all_lineages:
        lineage_counts = counts_table[counts_table.lineage == lineage]
...
# python MutationalSignatures/PseudoReferenceGenerator.py -a ../../../../home5/nCov/Richard/gisaid/20210907/7_aligned.fasta -r Resources/GISAID/Reference/GISAID_REFERENCE.gb -c 200 -b 16000 -th 8 -o pseudo_references.fasta